...
def crear_producto_shopify(datos_producto, variantes):
    ...
    # Crear opciones solo si hay variantes
    options = []
    if variantes:
        options = [
    ...
    payload = {
        "product": {
            ...
            "body_html": f"sku: {datos_producto['SKU']}",
            "vendor": "Automático",
            ...
            "variants": [
                {
                    "option1": variante.get("color", None),
                    "option2": variante.get("talla", None),
...
def verificar_producto_existente(sku):
    url = f"https://{SHOP_NAME}.myshopify.com/admin/api/2023-01/products.json?fields=id,body_html&limit=250"
    response = requests.get(url, headers=HEADERS)
    if response.status_code == 200:
        productos = response.json().get('products', [])
        for producto in productos:
            body_html = producto.get("body_html", "")
            match = re.search(r"sku:\s*(\S+)", body_html)
...
def subir_productos_a_shopify():
    ...
    for articulo in datos_articulos:
        ...
        # Crear el producto (con o sin variantes)
        creado = crear_producto_shopify(datos_producto, combinaciones)
        if creado:
            print(f"Producto '{datos_producto['NombreArticulo']}' subido exitosamente.")
        else:
            print(f"Error al subir el producto '{datos_producto['NombreArticulo']}'.")
